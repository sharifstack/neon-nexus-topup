...
export default function HomePageClient({ featuredGames, flashDeals }: HomePageClientProps) {
    ...
    <div className="max-w-container-max mx-auto px-gutter py-xxl flex flex-col gap-xxl w-full">
      ...
      <section className="relative w-full h-[500px] md:h-[600px] rounded-xl overflow-hidden glass-panel flex items-center p-xl md:p-xxl">
        ...
        <div className="relative z-20 flex flex-col items-start gap-md max-w-2xl">
          <div className="bg-tertiary text-on-tertiary-container font-label-md text-label-md px-sm py-xs rounded uppercase tracking-widest inline-block mb-sm">{t.badge}</div>
          <h1 className="font-display-lg-mobile md:font-display-lg text-display-lg-mobile md:text-display-lg text-primary leading-tight">{t.heroTitle1} <br/><span className="text-primary-container">{t.heroTitle2}</span></h1>
          <p className="font-body-lg text-body-lg text-on-surface-variant mb-lg">{t.heroSubtitle}</p>
          <Link href="/marketplace" className="bg-gradient-to-r from-primary-container to-[#00aaff] text-on-primary-container font-headline-md text-headline-md px-xl py-md rounded-lg btn-glow inline-block transition-all hover:scale-105 active:scale-95">{t.enterMarketplace}</Link>
        </div>
      </section>
        ...
        <section className="w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-lg">
            {flashDeals.map((game: any) => (
              <GameCard key={game._id} game={game} badge={t.flashDeal} badgeColor="bg-secondary" t={t} />
            ))}
          </div>
        </section>
      )}
      ...
      <section className="w-full">
        <div className="flex items-center justify-between mb-xl">
          <h2 className="font-headline-xl text-headline-xl text-primary">{t.categories}</h2>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-md md:gap-lg">
          <CategoryCard icon="sports_esports" label={t.battleRoyale} color="text-primary-container" />
          <CategoryCard icon="sports_soccer" label={t.sports} color="text-secondary" />
          <CategoryCard icon="swords" label={t.rpg} color="text-tertiary" />
          <CategoryCard icon="groups" label={t.moba} color="text-primary" />
        </div>
      </section>
      ...
      <section className="w-full">
        <div className="flex items-center justify-between mb-xl">
          <h2 className="font-headline-xl text-headline-xl text-primary">{t.featuredTitles}</h2>
...
function GameCard({ game, badge, badgeColor = "bg-primary", t }: { game: any; badge?: string; badgeColor?: string; t: any }) {
